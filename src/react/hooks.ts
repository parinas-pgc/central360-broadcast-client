 ...
 */
function inboxQueryKey(basePath: string, unreadOnly: boolean, pageSize: number): QueryKey {
  return ["broadcast", "inbox", basePath, { unreadOnly, pageSize }] as const;
}
function inboxQueryKeyPrefix(basePath: string): QueryKey {
  return ["broadcast", "inbox", basePath] as const;
}
 ...
 */
export function useBroadcastInbox(opts?: { unreadOnly?: boolean; pageSize?: number }) {
  const ctx = useBroadcastContext();
  const pageSize = opts?.pageSize ?? DEFAULT_PAGE_SIZE;
  const unreadOnly = opts?.unreadOnly ?? false;
  const pollMs = ctx.pollIntervalMs ?? DEFAULT_POLL_MS;

  const query = useInfiniteQuery<InboxPage, Error, InfiniteData<InboxPage>, QueryKey, string | undefined>({
  ...
  const broadcasts = useMemo<InboxBroadcast[]>(
    () => (query.data?.pages ?? []).flatMap((p) => p.broadcasts),
    [query.data],
  );
...
export function useImportantBanner(): InboxBroadcast | null {
...
export function useFirstLoginModals(): InboxBroadcast[] {
  const { broadcasts } = useBroadcastInbox();
 ...
 */
function patchAllInboxCaches(
  qc: ReturnType<typeof useQueryClient>,
  basePath: string,
  broadcastId: string,
  patch: Partial<InboxBroadcast>,
): CacheSnapshot[] {
  const entries = qc.getQueriesData<InfiniteData<InboxPage>>({ queryKey: inboxQueryKeyPrefix(basePath) });
  ...
  for (const [key, data] of entries) {
    ...
    const pages = data.pages.map((p) => ({
      ...
      broadcasts: p.broadcasts.map((b) => {
        ...
        if (patch.read_at && b.read_at === null && patch.read_at !== null) unreadDelta -= 1;
        return { ...b, ...patch };
      }),
    }));
    if (unreadDelta !== 0) {
...
function rollbackInboxCaches(
  qc: ReturnType<typeof useQueryClient>,
  snapshots: CacheSnapshot[],
): void {
  for (const snap of snapshots) qc.setQueryData(snap.key, snap.data);
}
 ...
 */
async function expectOk(p: Promise<boolean>): Promise<true> {
  const ok = await p;
  if (!ok) throw new Error("hub_write_failed");
  return true;
}
...
export function useMarkRead() {
  ...
  return useMutation<true, Error, string, CacheSnapshot[]>({
    ...
    onError: (_err, _id, snapshots) => {
      if (snapshots) rollbackInboxCaches(qc, snapshots);
    },
    onSettled: () => {
